...
plot_pca <- function(rld) {
    pcaData <- myPlotPCA(rld, intgroup = c('condition','sex','age'), returnData = TRUE)
    ...
    ggplot(pcaData, aes(x=PC1, y=PC2, color=condition)) +
        ...
        geom_text_repel(aes(label=paste(pcaData$name,'(', pcaData$sex,' - ', pcaData$age, ')', sep='')),
        ...
        theme(panel.grid.major = element_blank(),
              ...
              legend.key = element_rect(fill = "gray", linewidth=0.1),
...
my_volcano <- function(res, lfc.cut = 2, sig.cut = 0.05) {
    ...
    mydata.top <- row.names(mydata.top[order(mydata.top$padj),])
...
volcanoplot <- function (res, lfcthresh=0.7, sigthresh=0.05, main="Volcano Plot", legendpos="bottomright", labelsig=FALSE, textcx=1, ...) {
...
plot_heat <- function(rld,selected,gene_names,title) {
    ## The data
    ...
    anno <- as.data.frame(colData(rld)[, 'condition', drop=FALSE])#,'site','gender','lung','myositis','anti.scl'),
#                                       drop=FALSE])
