...
## ASSIGN SAMPLE NAMES
new_names <- c('CTRL01','CTRL02','CTRL03','CTRL04','CTRL05','CASE06','CASE07','CASE08','CASE09',
               'CTRL10','CASE11','CASE12','CTRL13','CTRL14','CASE15','CTRL16','CASE17','CTRL18',
               'CASE19','CASE20','CASE21','CASE22','CASE23','CTRL24','CASE25','CTRL26','CASE27',
               'CASE28','CTRL29','CASE30','CTRL31','CASE32','CASE33','CTRL34','CTRL35','CASE36',
...
compare.COMP <- list(c('CTRL01','CTRL02','CTRL03','CTRL04','CTRL05','CTRL10','CTRL13','CTRL14','CTRL16','CTRL18',
                       'CTRL24','CTRL26','CTRL29','CTRL31','CTRL34','CTRL35','CTRL37','CTRL40'),
                     c('CASE06','CASE07','CASE08','CASE09','CASE11','CASE12','CASE15','CASE17','CASE19',
                       'CASE20','CASE21','CASE22','CASE23','CASE25','CASE27','CASE28','CASE30','CASE32','CASE33',
                       'CASE36','CASE38','CASE39'))

## SAMPLE CONRITION
condition <- factor(c(rep('case',22),rep('ctrl',18))) ## MUST AUTOMATE - USER INPUT MAYBE
...
## SUBSET
the.data <- the.data[, !colnames(the.data) %in% c('CASE22','CASE30','CASE17','CASE23','CTRL01','CTRL02','CTRL26')]
the.coldata <- the.coldata[!row.names(the.coldata) %in% c('CASE22','CASE30','CASE17','CASE23','CTRL01','CTRL02','CTRL26'),]
...
## DO DIFF!
COMP <- DESeqDataSetFromMatrix(countData=the.data, colData=the.coldata, design = ~ condition)
COMP$condition <- relevel(COMP$sex, ref="M")
...
write.csv(genes.COMP, paste0(tables.out.dir,'genes_mvsf_0.5.csv'))

## PLOTS
pdf(paste0(figures.out.dir,'plot.pca_mvsf_0.5.pdf'), width = 7, height = 6, onefile=FALSE)
plot_pca(rld.COMP)
dev.off()

pdf(paste0(figures.out.dir,'plot.volcano_mvsf_0.5.pdf'), width = 7, height = 6, onefile=FALSE)
my_volcano(res.COMP,0.5,0.05)
dev.off()

pdf(paste0(figures.out.dir,'plot.heatmap_mvsf_0.5.pdf'),onefile=FALSE)
...
diff_expr_results <- function(sig.res,genes,dds){
    ...
    write.table(expr_table,file=paste0(tables.out.dir,"stats_mvsf_0.6.csv"), row.names=FALSE,sep="|")
    return(expr_table)
}
